...
run(function()
	...
	local function updateStore(newStore, oldStore)
		if newStore.GameCurrency ~= oldStore.GameCurrency then
			riseEvents.CurrencyChange:Fire(table.clone(newStore.GameCurrency.Quantities))
		end
	...
	rise:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	rise:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
...
run(function()
	...
	InvMove = rise.Categories.Blatant:CreateModule({
		Name = 'InvMove',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, armC0 = rise.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and rise.ThreadFix then
						setthreadidentity(8)
					end
...
run(function()
	...
	AutoBuy = rise.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoBuy:Clean(riseEvents.CurrencyChange.Event:Connect(buyCheck))
	...
	Viewmodel = rise.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then 
				...
				rise:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then 
