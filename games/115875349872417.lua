...
local drawingactor = loadstring(downloadFile('rise/libraries/drawing.lua'), 'drawing')(...)
local redline = {Teams = {}}
...
local function warningRoutine(hash)
	local path = 'rise/profiles/agreementhash.txt'
...
if not select(1, ...) then
	if run_on_actor then
		local oldreload = shared.risereload
		rise.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.rise
			local executionString = "loadfile('rise/main.lua')("..drawingactor..")"
			...
			if oldreload then
				executionString = 'shared.risereload = true\n'..executionString
			end
		...
		rise.Load = function()
			notif('Rise', 'Missing actor functions.', 10, 'alert')
		end
	end
	...
	if game.PlaceId == 126691165749976 then
		...
		local function processPlayer(plr)
			if tonumber(plr.Name) then
				...
				rise:Clean(plr:GetAttributeChangedSignal('team_id'):Connect(function()
					redline.Teams[plr.Name] = plr:GetAttribute('team_id')
					task.spawn(updatePlayer, plr)
				end))
			end
		...
		local function processMatch(match)
			if match and match.Name == 'Match' then
				rise:Clean(match.DescendantAdded:Connect(processPlayer))
		...
		rise:Clean(replicatedStorage.ReadOnly.ChildAdded:Connect(processMatch))
...
run(function()
	...
	for _, v in getloadedmodules() do
		if v:GetFullName() == 'Start.Client.ClientRoot' then
			...
			if not rawget(root, 'loaded') then
				repeat
					task.wait()
				until rawget(root, 'loaded') or rise.Loaded == nil
			end
	...
	if redline.ActionEventPacket then
		rise:Clean(redline.ActionEventPacket.OnClientEvent:Connect(function(data)
	...
	rise:Clean(riseEvents.MatchEnded.Event:Connect(function(won)
		if won then
			wins:Increment()
		end
	end))

	rise:Clean(lplr.PlayerGui.ChildAdded:Connect(function(obj)
		if obj.Name == 'MatchResultsScreen' then
			...
			if obj then
				...
				riseEvents.MatchEnded:Fire(obj.Text:find('WON') and true or false, results)
			end
		end
	end))
end)
...
run(function()
	...
	AntiParry = rise.Categories.Blatant:CreateModule({
		Name = 'AntiParry',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoParry = rise.Categories.Blatant:CreateModule({
		Name = 'AutoParry',
		Function = function(callback)
			if callback then
...
run(function()
	...
	InfiniteDash = rise.Categories.Blatant:CreateModule({
		Name = 'InfiniteDash',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoQueue = rise.Categories.Utility:CreateModule({
		Name = 'AutoQueue',
		Function = function(callback)
			if callback then
				AutoQueue:Clean(riseEvents.MatchEnded.Event:Connect(function(_, obj)
					task.defer(function()
...
run(function()
	...
	AutoToxic = rise.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoToxic:Clean(riseEvents.MatchEnded.Event:Connect(function(won)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
							if textChatService:CanUserChatAsync(lplr.UserId) then
...
run(function()
	...
	HitSound = rise.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then
				HitSound:Clean(riseEvents.Hit.Event:Connect(function()
					if #sounds > 0 then
...
run(function()
	...
	KillSound = rise.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then
				KillSound:Clean(riseEvents.PlayerKill.Event:Connect(function()
					if #sounds > 0 then
						local obj = Instance.new('Sound')
						obj.SoundId = sounds[math.random(1, #sounds)]
						obj.PlayOnRemove = true
