...
local uipallet = rise.Libraries.uipallet
local tween = rise.Libraries.tween
local color = rise.Libraries.color
...
shared.CRYSTALRISE_STORE = store
...
local function isFriend(plr, recolor)
	if rise.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(rise.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and rise.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(rise.Categories.Targets.ListEnabled, plr.Name) and true
end

local function notif(...) return
	rise:CreateNotification(...)
end
...
run(function()
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						riseEvents.AttributeChanged:Fire(attr)
					end))
				else
	...
	rise:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
end)
entitylib.start()
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Rise', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not rise.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			rise:Clean(connection:Connect(function(...)
				riseEvents[event]:Fire(...)
			end))
		end)
	end

	rise:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		riseEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		rise:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or rise.Loaded == nil
			if rise.Loaded == nil then return end
			mapname = workspace:WaitForChild('Map', 5):WaitForChild('Worlds', 5):GetChildren()[1].Name
			mapname = string.gsub(string.split(mapname, '_')[2] or mapname, '-', '') or 'Blank'
		end)
	end)

	rise:Clean(riseEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	rise:Clean(riseEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	rise:Clean(riseEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until rise.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
for _, v in {'AntiRagdoll', 'SilentAim', 'AutoRejoin', 'Rejoin', 'Disabler', 'Timer', 'ServerHop', 'MouseTP', 'MurderMystery'} do
	rise:Remove(v)
end
run(function()
	local AimAssist
	...
	AimAssist = rise.Categories.Combat:CreateModule({
		Name = 'AimAssist',
...
run(function()
	...
	AutoClicker = rise.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	rise.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Reach = rise.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
...
run(function()
	...
	Sprint = rise.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then 
...
run(function()
	...
	Velocity = rise.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AntiFall = rise.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if rise.Modules.Fly.Enabled or (rise.Modules.InfiniteFly and rise.Modules.InfiniteFly).Enabled or rise.Modules.LongJump.Enabled then
...
run(function()
    ...
    AntiVoidPlus = rise.Categories.Blatant:CreateModule({
        Name = "AntiVoid+",
            ...
            AntiVoidPlus:Clean(runService.PreSimulation:Connect(function(dt)
                ...
                if root.Position.Y < FloorY + 5 then
                    if AntiVoidPlus.Mode.Value == "Smooth" then
                        ...
                        if rise.Modules.BetterFly and rise.Modules.BetterFly.Enabled then
...
run(function()
	...
	FastBreak = rise.Categories.Blatant:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Fly = rise.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(riseEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	HitBoxes = rise.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
...
run(function()
	rise.Categories.Blatant:CreateModule({
		Name = 'KeepSprint',
		Function = function(callback)
...
run(function()
	...
	Killaura = rise.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				if inputService.TouchEnabled then
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function(_, _, dir, Value)
			LongJump:Clean(riseEvents.CatPounce.Event:Connect(function()
				JumpSpeed = Value.Value
				JumpTick = tick() + 2.5
				Direction = Vector3.new(dir.X, 0, dir.Z).Unit
				entitylib.character.RootPart.Velocity = Vector3.zero
	...
	LongJump = rise.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			frictionTable.LongJump = callback or nil
			updateVelocity()
			if callback then
				LongJump:Clean(riseEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(riseEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
    ...
    LongJumpPlus = rise.Categories.Blatant:CreateModule({
        Name = "LongJump+",
        Function = function(callback)
            ...
            LongJumpPlus:Clean(riseEvents.EntityDamageEvent.Event:Connect(function(damageTable)
...
run(function()
	...
	NoFall = rise.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				local tracked = 0
...
run(function()
	...
	local ProjectileAimbot = rise.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = rise.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Speed = rise.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
    ...
    BetterSpeed = rise.Categories.Blatant:CreateModule({
        Name = 'Speed +',
        Function = function(callback)
...
run(function()
	...
	BedESP = rise.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Health = rise.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
				...
				label.Parent = rise.gui
				Health:Clean(label)
				Health:Clean(riseEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = rise.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = rise.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(rise.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	StorageESP = rise.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = rise.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = rise.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPearl = rise.Categories.Utility:CreateModule({
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	AutoPlay = rise.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(riseEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(riseEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	rise.Categories.Utility:CreateModule({
		Name = 'AutoShoot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoToxic = rise.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(riseEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(riseEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(riseEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = rise.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	PickupRange = rise.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Scaffold = rise.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if label then
...
run(function()
	...
	ShopTierBypass = rise.Categories.Utility:CreateModule({
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function staffFunction(plr, checktype)
		if not rise.Loaded then
			repeat task.wait() until rise.Loaded
		end
		...
		elseif Mode.Value == 'Profile' then
			...
			if rise.Profile ~= Profile.Value then
				rise:Load(true, Profile.Value)
			end
		elseif Mode.Value == 'AutoConfig' then
			local safe = {'AutoClicker', 'Reach', 'Sprint', 'HitFix', 'StaffDetector'}
			rise.Save = function() end
			for i, v in rise.Modules do
	...
	local function playerAdded(plr)
		...
		elseif getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and rise.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	StaffDetector = rise.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
	...
	task.spawn(function()
		repeat task.wait(1) until rise.Loaded or rise.Loaded == nil
		if rise.Loaded and not StaffDetector.Enabled then
...
run(function()
	TrapDisabler = rise.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
...
run(function()
	rise.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoSuffocate = rise.Categories.World:CreateModule({
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTool = rise.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = rise.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChestSteal = rise.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = rise.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ArmorSwitch = rise.Categories.Inventory:CreateModule({
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBank = rise.Categories.Inventory:CreateModule({
		Name = 'AutoBank',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = rise.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				repeat task.wait() until store.queueType ~= 'bedwars_test'
...
run(function()
	...
	AutoConsume = rise.Categories.Inventory:CreateModule({
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(riseEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
				AutoConsume:Clean(riseEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = rise.gui.ScaledGui
		...
		close.Image = getcustomasset('rise/assets/new/close.png')
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			rise.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		searchicon.Image = getcustomasset('rise/assets/new/search.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / rise.guiscale.Scale)
		end)
	...
	rise.Components.HotbarList = function(optionsettings, children, api)
		if rise.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('rise/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / rise.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					rise.gui.ScaledGui.ClickGui.Visible = false
			...
			close.BackgroundColor3 = Color3.new(1, 1, 1)
			close.BackgroundTransparency = 1
			close.Image = getcustomasset('rise/assets/new/closemini.png')
			close.ImageColor3 = color.Light(uipallet.Text, 0.2)
			close.ImageTransparency = 0.5
			close.AutoButtonColor = false
			close.Parent = hotbar
	...
	AutoHotbar = rise.Categories.Inventory:CreateModule({
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(riseEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = rise.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = rise.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
	...
	BedPlates = rise.Categories.Minigames:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				BedPlates:Clean(riseEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(riseEvents.BreakBlockEvent.Event:Connect(refreshNear))
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('rise/assets/new/blur.png'),
...
run(function()
	...
	BedBreakEffect = rise.Legit:CreateModule({
		Name = 'Bed Break Effect',
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(riseEvents.BedwarsBedBreak.Event:Connect(function(data)
...
run(function()
	...
	local Crosshair = rise.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = rise.Legit:CreateModule({
...
run(function()
	...
	FPSBoost = rise.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = rise.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = rise.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = rise.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = rise.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = rise.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SoundChanger = rise.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = rise.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do
...
run(function()
	...
	Viewmodel = rise.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
...
run(function()
	...
	WinEffect = rise.Legit:CreateModule({
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
				WinEffect:Clean(riseEvents.MatchEndEvent.Event:Connect(function()
...
run(function()
	...
	ItemTP = rise.Categories.Utility:CreateModule({
		Name = 'ItemTP',
		Function = function(callback)
			if callback then
				ItemTP:Toggle()
...
run(function() -- rise.gg
 	local Explosions
        
    Explosions = rise.Categories.Utility:CreateModule({
        Name = 'Explosions',
...
run(function()
    ...
    BetterFly = rise.Categories.Blatant:CreateModule({
        Name = 'Fly +',
        Function = function(callback)
...
run(function()
	...
	LagbackNotifier = rise.Categories.Utility:CreateModule({
        ...
        Function = function(enabled)
            if enabled then
                ...
                LagbackNotifier:Clean(lplr:GetAttributeChangedSignal('LastTeleported'):Connect(function()
                    rise:CreateNotification('1', 'Teleport detected', 3)
                end))
                    ...
                    if hrp then
                        if lastnetowner ~= isnetworkowner(hrp) then
                            ...
                            if not lastnetowner then
                                rise:CreateNotification('2', 'Lagback detected', 3)
                            end
                        end
                    end
                end))
...
run(function()
    ...
    AntiLagback = rise.Categories.Utility:CreateModule({
        Name = 'AntiLagback',
        Function = function(enabled)
                ...
                if lastHRPPos then
                    ...
                    if distance > lagThreshold and tick() - lastFix > cooldown then
                        ...
                        if lastSafeCFrame then
                            ...
                            rise:CreateNotification('AntiLagback', 'Lagback prevented!', 1.5)
                        end
                    end
                end
...
run(function()
    ...
    EmoteSpammer = rise.Categories.Utility:CreateModule({
        Name = 'EmoteSpammer',
        Tooltip = '',
        Function = function(enabled)
            if enabled then
...
run(function()
    ...
    Disabler = rise.Categories.Utility:CreateModule({
        Name = 'Disabler',
            ...
            repeat
                ...
                elseif hammer then
                    ...
                    if bedwars.AbilityController:canUseAbility(hammer.itemType..'_jump') and Disabler.Enabled then
                        ...
                        rise:CreateNotification("Disabler", "Temporarily disabled", hammer.tool.Name == 'jade_hammer' and 2.8 or .8) -- 3.6 total, 2.8 safe
                    end
                elseif getItem('hang_glider') then
                    ...
                    for _, glider in (workspace:FindFirstChild("Gliders") or Instance.new("Folder")):GetChildren() do -- TODO check if its in every game
                        if glider:GetAttribute("GliderOwner") == lplr.UserId then
                            glider:Destroy()
                            rise:CreateNotification("Disabler", "Successfully disabled!", 6)
                        end
                    end
                    repeat
                        local root = entitylib.character.RootPart
					...
					if getItem('flying_backpack') then
						if RemotesInstance.FlyingBackpackFlap:InvokeServer() then
							rise:CreateNotification("Disabler", "Float disabled for 2.8s", 2.8)
						end
					end
...
run(function()
    local ZoomUnlocker
    ZoomUnlocker = rise.Categories.Utility:CreateModule({
        Name = "Zoom Unlocker",
        Function = function(callback)
	    	if callback then
...
run(function()
    local InfiniteJump
    local Velocity
    InfiniteJump = rise.Categories.Utility:CreateModule({
        Name = "InfiniteJump",
        Function = function(callback)
            if callback then
...
run(function()
    ...
    AutoBuyWool = rise.Categories.Inventory:CreateModule({
        Name = 'Wool buyer',
...
run(function()
	...
	DamageIndicator = rise.Categories.Render:CreateModule({
...
run(function()
    Furryall = rise.Categories.Blatant:CreateModule({
        Name = 'Furryall',
        Function = function(callback)
...
run(function()
    ...
    Headless = rise.Categories.Utility:CreateModule({
        Name = "Headless",
        Function = function(enabled)
            if enabled then
...
run(function()
    ...
    StreamerMode = rise.Categories.Utility:CreateModule({
        Name = 'StreamerMode',
        Function = function(callback)
...
run(function()
	...
	SkyScytheKill = rise.Categories.Utility:CreateModule({
...
run(function()
    ...
    NightmareEmote = rise.Categories.Utility:CreateModule({
...
run(function()
    ...
    local NoNameTag = rise.Categories.Utility:CreateModule({
        Name = 'NoNameTag',
        Function = function(callback)
            if callback then
...
run(function()
    ...
    local TexturePacks 
	TexturePacks = rise.Categories.Render:CreateModule({
        Name = 'TexturePacks',
