...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		...
		task.spawn(function()
			...
			notif('Rise', 'Failed to find actor', 10, 'alert')
		end)
	else
		rise.Load = function()
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('Rise', 'Failed to hook ('..id..')', 10, 'alert')
	end
...
run(function()
	...
	rise:Clean(Drawing.kill or function() end)
	rise:Clean(function()
...
run(function()
	...
	entitylib.getEntityColor = function(ent)
		if not (ent.Player and rise.Settings.Modules.Options['Use team color'].Enabled) then return end
...
run(function()
	...
	GrenadeTP = rise.Categories.Blatant:CreateModule({
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	GrenadeESP = rise.Categories.Render:CreateModule({
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoHurtCam = rise.Categories.Render:CreateModule({
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = rise.Categories.Render:CreateModule({
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = rise.Categories.Utility:CreateModule({
		Name = 'AutoRespawn',
		Function = function(callback)
			if callback then
