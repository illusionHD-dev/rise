...
run(function()
	...
	blocks = collection('BedWarsX_PlacedBlock', rise, function(tab, block)
	...
	rise:Clean(lplr:GetAttributeChangedSignal('RoundKills'):Connect(function()
		if lplr:GetAttribute('RoundKills') > 0 then
			kills:Increment()
		end
	end))

	rise:Clean(bw.RemoteIndex.Round_Event.OnClientEvent:Connect(function(data)
	...
	rise:Clean(bw.RemoteIndex.Bed_Destroyed.OnClientEvent:Connect(function(data)
		if type(data) == 'table' and data.breakerId == lplr.UserId then
			beds:Increment()
		end
	end))

	rise:Clean(entitylib.Events.EntityAdded:Connect(function(entity)
	...
	rise:Clean(entitylib.Events.EntityRemoving:Connect(function(entity)
...
run(function()
	...
	AnticheatBypass = rise.Categories.Blatant:CreateModule({
...
run(function()
	...
	AutoLeave = rise.Categories.Utility:CreateModule({
		Name = 'AutoLeave',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastBreak = rise.Categories.World:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HideShield = rise.Legit:CreateModule({
		Name = 'HideShield',
		Function = function(callback)
			if callback then
