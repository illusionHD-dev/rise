local loadstring = function(...)
	...
	if err and rise then rise:CreateNotification('Rise', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/illusionHD-dev/rise/'..readfile('rise/profiles/commit.txt')..'/'..select(1, path:gsub('rise/', '')), true) end)
		if not suc or res == '404: Not Found' then error(res) end
		if path:find('.lua') then res = '--This watermark is used to delete the file if its cached, remove it to make the file persist after rise updates.\n'..res end
		writefile(path, res)
	end
...
local vm = loadstring(downloadFile('rise/libraries/vm.lua'), 'vm')()
...
run(function()
		...
		if plr and plr:IsA('Player') then
			...
			self.Connections[inventory] = {
				inventory.ChildAdded:Connect(function(tool)
					...
					if plr == lplr then
						riseEvents.ItemAdded:Fire(tool)
					else
	...
	rise:Clean(collectionService:GetInstanceAddedSignal('Inventory'):Connect(function(inventory)
		InvTracker:AddInventory(inventory)
	end))
...
run(function()
	...
	rise:Clean(replicatedStorage.BountyData:GetPropertyChangedSignal('Value'):Connect(function()
...
run(function()
	...
	if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
		repeat
			task.wait()
		until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or rise.Loaded == nil
	...
	local function FireServerHook(...)
		...
		if remote ~= 'LookAngle' and remote ~= 'AimPosition' and shared.RiseDeveloper then
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('Rise', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
...
run(function()
	...
	AutoEject = rise.Categories.Blatant:CreateModule({
		Name = 'AutoEject',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoPickpocket = rise.Categories.Blatant:CreateModule({
		Name = 'AutoPickpocket',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoPop = rise.Categories.Blatant:CreateModule({
		Name = 'AutoPop',
		Function = function(callback)
			if callback then
				task.spawn(function()
...
run(function()
	...
	AutoPunch = rise.Categories.Blatant:CreateModule({
		Name = 'AutoPunch',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTaze = rise.Categories.Blatant:CreateModule({
		Name = 'AutoTaze',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ForceEquip = rise.Categories.Blatant:CreateModule({
		Name = 'ForceEquip',
		Function = function(callback)
			if callback then
...
run(function()
	...
	LazerGodmode = rise.Categories.Blatant:CreateModule({
		Name = 'LazerGodmode',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoSlowdown = rise.Categories.Blatant:CreateModule({
		Name = 'NoSlowdown',
		Function = function(callback)
...
run(function()
	...
	InfNitro = rise.Categories.Utility:CreateModule({
		Name = 'InfiniteNitro',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoHotbar = rise.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				AutoHotbar:Clean(riseEvents.ItemAdded.Event:Connect(DoSorting))
...
run(function()
	...
	FPSBooster = rise.Legit:CreateModule({
		Name = 'FPSBooster',
		Function = function(callback)
			if callback then
