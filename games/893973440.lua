...
run(function()
	...
	if not lstats then
		repeat
			lstats = lplr:FindFirstChild('TempPlayerStatsModule')
			task.wait()
		until lstats or rise.Loaded == nil
	...
	local function updateMap()
		if mapval.Value then
			mapobj = mapval.Value
			riseEvents.MapAdded:Fire(mapobj)
		elseif mapboj then
			riseEvents.MapRemoved:Fire(mapboj)
			mapobj = nil
		end
	end

	rise:Clean(mapval:GetPropertyChangedSignal('Value'):Connect(updateMap))
	if mapval.Value then
		updateMap()
	end
end)
...
run(function()
	...
	PhaseHammer = rise.Categories.Blatant:CreateModule({
		Name = 'PhaseHammer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	RestrainBeast = rise.Categories.Blatant:CreateModule({
		Name = 'RestrainBeast',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SlowBeast = rise.Categories.Blatant:CreateModule({
		Name = 'SlowBeast',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SpamBeast = rise.Categories.Blatant:CreateModule({
		Name = 'SpamBeast',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ComputerESP = rise.Categories.Render:CreateModule({
		Name = 'ComputerESP',
		Function = function(callback)
			if callback then
				ComputerESP:Clean(riseEvents.MapAdded.Event:Connect(MapAdded))
				ComputerESP:Clean(riseEvents.MapRemoved.Event:Connect(function()
...
run(function()
	...
	AutoComputer = rise.Categories.Utility:CreateModule({
		Name = 'AutoComputer',
		Function = function(callback)
			if callback then
