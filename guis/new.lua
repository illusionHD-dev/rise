...
mainapi.Libraries = {
	...
	getriseasset = getcustomasset,
	getfontsize = getfontsize,
	getfontbounds = getfontsize,
	tween = tween,
	uipallet = uipallet,
}
...
components = {
	...
	Bind = function(optionsettings, children, api)
		local optionapi = {
			Type = 'Bind',
			Bind = table.clone(optionsettings.Default or {}),
			Hold = optionsettings.Hold or false,
			Index = getTableSize(api.Options)
		}
		local bind = Instance.new('TextButton')
		bind.Name = optionsettings.Name..'Bind'
		bind.Size = UDim2.new(1, 0, 0, 28)
		bind.BackgroundTransparency = 1
		bind.Text = ''
		bind.Visible = optionsettings.Visible == nil or optionsettings.Visible
		bind.Parent = children
		local title = Instance.new('TextLabel')
		title.Size = UDim2.new(1, -13, 1, 0)
		title.Position = UDim2.fromOffset(13 + (optionsettings.Darker and 20 or 0), 0)
		title.BackgroundTransparency = 1
		title.Text = optionsettings.Name..': '..table.concat(optionapi.Bind, ' + ')
		title.TextColor3 = color.Dark(uipallet.Text, 0.21)
		title.TextSize = 18
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.FontFace = uipallet.Font
		...
		function optionapi:SetBind(keys, mouse)
			self.Bind = table.clone(keys or {})
			title.Text = optionsettings.Name..': '..(#self.Bind > 0 and table.concat(self.Bind, ' + ') or 'None')
		end

		function optionapi:Save(tab)
			tab[optionsettings.Name] = {Bind = self.Bind, Hold = self.Hold}
		end

		function optionapi:Load(tab)
			self.Hold = tab.Hold == true
			self:SetBind(tab.Bind or {})
		end

		bind.MouseButton1Click:Connect(function()
			if inputService:IsKeyDown(Enum.KeyCode.LeftShift) then
				mainapi.Binding = self
			end
		end)

		optionapi.Object = bind
		api.Options[optionsettings.Name] = optionapi
		return optionapi
	end,
...
function mainapi:CreateCategory(categorysettings)
	...
	function categoryapi:CreateModule(modulesettings)
		local moduleapi = {
			...
			Description = modulesettings.Tooltip or '',
		...
		modulebutton.InputBegan:Connect(function(inputObj)
			if inputObj.UserInputType == Enum.UserInputType.MouseButton3 then
				mainapi.Binding = moduleapi
				mainapi:CreateNotification('Bind', 'Press a key to bind '..moduleapi.Name, 2)
			end
		end)
		...
		if mainapi.RefreshSearch then
			mainapi:RefreshSearch()
		end
...
function mainapi:Save(newprofile)
	...
	if newprofile then
		...
		for _, profile in self.Profiles do
			if profile.Name == newprofile then
				found = true
				break
			end
		end
		if not found then
			table.insert(self.Profiles, {Name = newprofile, Bind = {}})
		end
	...
	writefile('rise/profiles/'..targetProfile..self.Place..'.txt', httpService:JSONEncode(savedata))
end
...
makeDraggable(draghandle, clickgui)
...
local cascategory = mainapi:CreateCategoryProfile({
	Name = 'CaS',
	RealName = 'Profiles',
	RiseIcon = 'm',
	Font = 3,
...
do
	local content = cascategory.Children
	local function casLabel(parent, text, position, size, textsize, textcolor, centered)
		local object = Instance.new('TextLabel')
		object.BackgroundTransparency = 1
		object.Position = position
		object.Size = size
		object.Text = text
		object.TextColor3 = textcolor or uipallet.Text
		object.TextSize = textsize
		object.TextXAlignment = centered and Enum.TextXAlignment.Center or Enum.TextXAlignment.Left
		object.FontFace = uipallet.Font
		object.Parent = parent
		return object
	end

	local function casCard(profile, position)
		local card = Instance.new('TextButton')
		card.Name = profile.Name
		card.Size = UDim2.fromOffset(174, 174)
		card.Position = position
		card.BackgroundColor3 = color.Dark(uipallet.Main, 0.03)
		card.AutoButtonColor = false
		card.Text = ''
		card.Parent = content
		addCorner(card, UDim.new(0, 16))
		casLabel(card, profile.Name, UDim2.fromOffset(0, 66), UDim2.new(1, 0, 0, 30), 21, uipallet.Text, true)
		casLabel(card, 'Click to load', UDim2.fromOffset(0, 101), UDim2.new(1, 0, 0, 24), 16, color.Dark(uipallet.Text, 0.43), true)
		card.MouseEnter:Connect(function() card.BackgroundColor3 = color.Dark(uipallet.Main, 0.07) end)
		card.MouseLeave:Connect(function() card.BackgroundColor3 = color.Dark(uipallet.Main, 0.03) end)
		card.MouseButton1Click:Connect(function()
			mainapi:Load(true, profile.Name == 'Latest' and (mainapi.Profile or 'default') or profile.Name)
		end)
	end

	local function refreshCaS()
		for _, object in content:GetChildren() do
			if object:IsA('GuiObject') then object:Destroy() end
		end
		local featured = Instance.new('Frame')
		featured.Name = 'FeaturedBanner'
		featured.Size = UDim2.new(1, -10, 0, 220)
		featured.BackgroundColor3 = color.Dark(uipallet.Main, 0.08)
		featured.BorderSizePixel = 0
		featured.Parent = content
		addCorner(featured, UDim.new(0, 18))
		casLabel(content, 'Featured Configs  0', UDim2.fromOffset(0, 260), UDim2.new(1, 0, 0, 30), 19)
		local configs = {}
		for _, profile in mainapi.Profiles do
			if profile.Name ~= 'default' then table.insert(configs, profile) end
		end
		casLabel(content, 'Your Configs  '..(#configs + 1), UDim2.fromOffset(0, 340), UDim2.new(1, 0, 0, 30), 19)
		casCard({Name = 'Latest'}, UDim2.fromOffset(0, 400))
		for index, profile in configs do
			local x = ((index - 1) % 3) * 188
			local y = 400 + (math.floor((index - 1) / 3) * 188)
			casCard(profile, UDim2.fromOffset(x, y))
		end
		content.CanvasSize = UDim2.fromOffset(0, 590 + (math.ceil(#configs / 3) * 188))
	end

	cascategory.Object.MouseButton1Click:Connect(refreshCaS)
	mainapi.RefreshCaS = refreshCaS
	refreshCaS()
end
...
do
	local searchcategory = mainapi.Categories.Search
	if searchcategory and searchcategory.Children then
		local searchholder = Instance.new('Frame')
		searchholder.Name = 'SearchBar'
		searchholder.Size = UDim2.fromOffset(566, 54)
		searchholder.BackgroundTransparency = 1
		searchholder.Parent = searchcategory.Children
		...
		searchbox.PlaceholderColor3 = color.Dark(uipallet.Text, 0.45)
		searchbox.Text = ''
		searchbox.TextColor3 = uipallet.Text
		searchbox.TextSize = 23
		searchbox.TextXAlignment = Enum.TextXAlignment.Center
		searchbox.Parent = searchholder

		local results = Instance.new('ScrollingFrame')
		results.Name = 'SearchResults'
		results.Size = UDim2.fromOffset(566, 0)
		results.BackgroundTransparency = 1
		results.BorderSizePixel = 0
		results.ScrollBarThickness = 2
		results.ScrollBarImageColor3 = color.Dark(uipallet.Text, 0.56)
		results.Parent = searchcategory.Children
		...
		empty.Name = 'Empty'
		empty.Size = UDim2.fromOffset(566, 80)
		empty.BackgroundTransparency = 1
		empty.Text = 'Type to search modules'
		empty.TextColor3 = color.Dark(uipallet.Text, 0.48)
		empty.TextSize = 19
		empty.FontFace = uipallet.FontLight
		empty.Parent = results

		local function matches(module, query)
			local words = string.split(string.lower(query), ' ')
			local haystack = string.lower(module.Name..' '..(module.Category or ''))
			for _, word in words do
				if word ~= '' and not string.find(haystack, word, 1, true) then
					return false
				end
			end
			return query ~= ''
		...
		function mainapi:RefreshSearch()
			for _, object in results:GetChildren() do
				if object:IsA('GuiObject') and object ~= empty then
					object:Destroy()
				end
			end
			...
			if query == '' then
				empty.Text = 'Type to search modules'
				empty.Visible = true
				results.Size = UDim2.fromOffset(566, 80)
				results.CanvasSize = UDim2.fromOffset(0, 80)
				return
			end
			local found = {}
			for _, module in mainapi.Modules do
				if matches(module, query) then
			...
			empty.Text = #found == 0 and 'No modules found' or ''
			empty.Visible = #found == 0
			empty.Size = UDim2.fromOffset(566, #found == 0 and 80 or 0)
			for index, module in found do
				local result = Instance.new('TextButton')
				result.Name = module.Name..'SearchResult'
				result.LayoutOrder = index
				result.Size = UDim2.fromOffset(566, 76)
				result.AutoButtonColor = false
				result.BackgroundColor3 = color.Dark(uipallet.Main, module.Enabled and 0.08 or 0.03)
				result.BorderSizePixel = 0
				result.Parent = results
				addCorner(result, UDim.new(0, 12))
				local title = Instance.new('TextLabel')
				title.Size = UDim2.new(1, -26, 0, 25)
				title.Position = UDim2.fromOffset(13, 10)
				title.BackgroundTransparency = 1
				title.Text = module.Name
				title.TextColor3 = module.Enabled and mainapi:RiseColor(result.AbsolutePosition) or color.Dark(uipallet.Text, 0.12)
				title.TextSize = 22
				title.TextXAlignment = Enum.TextXAlignment.Left
			...
			metadata.Size = UDim2.new(1, -26, 0, 22)
			metadata.Position = UDim2.fromOffset(13, 43)
			metadata.BackgroundTransparency = 1
				metadata.Text = module.Category
				metadata.TextColor3 = color.Dark(uipallet.Text, 0.52)
			metadata.TextSize = 16
			metadata.TextXAlignment = Enum.TextXAlignment.Left
			metadata.FontFace = uipallet.FontLight
				metadata.Parent = result
				local description = Instance.new('TextLabel')
				description.Size = UDim2.new(1, -26, 0, 22)
				description.Position = UDim2.fromOffset(13, 43)
				description.BackgroundTransparency = 1
				description.Text = module.Description or ''
				description.TextColor3 = color.Dark(uipallet.Text, 0.52)
				description.TextSize = 16
				description.TextXAlignment = Enum.TextXAlignment.Left
				description.FontFace = uipallet.FontLight
				description.TextTruncate = Enum.TextTruncate.AtEnd
				description.Parent = result
				metadata.Position = UDim2.fromOffset(13 + getfontsize(module.Name, 22, uipallet.Font).X + 10, 13)
				metadata.Size = UDim2.new(1, -metadata.Position.X.Offset - 13, 0, 22)
				metadata.Text = '('..module.Category..')'
				result.MouseEnter:Connect(function()
					result.BackgroundColor3 = color.Dark(uipallet.Main, 0.08)
				end)
			...
			results.Size = UDim2.fromOffset(566, math.max(resultlayout.AbsoluteContentSize.Y + 10, 80))
			results.CanvasSize = UDim2.fromOffset(0, resultlayout.AbsoluteContentSize.Y + 10)
		end

		searchbox:GetPropertyChangedSignal('Text'):Connect(function()
			mainapi:RefreshSearch()
		end)
		mainapi.SearchBar = searchbox
		mainapi.SearchResults = results
		mainapi:RefreshSearch()
	end
end
...
do
	...
	panel.Name = 'ConfigsAndScripts'
	panel.Size = UDim2.fromOffset(620, 520)
	panel.Position = UDim2.fromScale(0.5, 0.5)
	panel.AnchorPoint = Vector2.new(0.5, 0.5)
	panel.BackgroundColor3 = color.Dark(uipallet.Main, 0.03)
	...
	paneltitle.Size = UDim2.new(1, -40, 0, 34)
	paneltitle.Position = UDim2.fromOffset(20, 16)
	paneltitle.BackgroundTransparency = 1
	paneltitle.Text = 'Configs & Scripts'
	paneltitle.TextColor3 = uipallet.Text
	paneltitle.TextSize = 24
	paneltitle.TextXAlignment = Enum.TextXAlignment.Left
	paneltitle.FontFace = uipallet.FontSemiBold
	...
	close.Text = '×'
	close.TextColor3 = color.Dark(uipallet.Text, 0.35)
	close.TextSize = 25
	...
	command.Name = 'CommandInput'
	command.Size = UDim2.new(1, -40, 0, 42)
	command.Position = UDim2.fromOffset(20, 60)
	command.BackgroundColor3 = color.Dark(uipallet.Main, 0.08)
	command.BackgroundTransparency = 0
	command.BorderSizePixel = 0
	command.ClearTextOnFocus = false
	command.FontFace = uipallet.Font
	command.PlaceholderText = 'Type a command, for example .config save pvp'
	command.PlaceholderColor3 = color.Dark(uipallet.Text, 0.45)
	command.Text = ''
	command.TextColor3 = uipallet.Text
	command.TextSize = 17
	command.TextXAlignment = Enum.TextXAlignment.Left
	command.ZIndex = 21
	command.Parent = panel
	addCorner(command, UDim.new(0, 10))
	local feedback = Instance.new('TextLabel')
	feedback.Size = UDim2.new(1, -40, 0, 24)
	feedback.Position = UDim2.fromOffset(20, 108)
	feedback.BackgroundTransparency = 1
	feedback.Text = 'Press . anywhere to open commands'
	feedback.TextColor3 = color.Dark(uipallet.Text, 0.48)
	feedback.TextSize = 15
	feedback.TextXAlignment = Enum.TextXAlignment.Left
	feedback.FontFace = uipallet.FontLight
	feedback.ZIndex = 21
	feedback.Parent = panel
	local cards = Instance.new('ScrollingFrame')
	cards.Name = 'Cards'
	cards.Size = UDim2.new(1, -40, 1, -150)
	cards.Position = UDim2.fromOffset(20, 140)
	cards.BackgroundTransparency = 1
	cards.BorderSizePixel = 0
	cards.ScrollBarThickness = 2
	cards.ScrollBarImageColor3 = color.Dark(uipallet.Text, 0.56)
	cards.ZIndex = 21
	cards.Parent = panel
	...
	local function card(text, subtext, callback, order)
		local object = Instance.new('TextButton')
		object.Size = UDim2.new(1, -4, 0, 64)
		object.LayoutOrder = order
		object.AutoButtonColor = false
		object.BackgroundColor3 = color.Dark(uipallet.Main, 0.08)
		object.BorderSizePixel = 0
		object.Text = ''
		object.ZIndex = 22
		object.Parent = cards
		addCorner(object, UDim.new(0, 11))
		local title = Instance.new('TextLabel')
		...
		subtitle.Position = UDim2.fromOffset(12, 35)
		subtitle.Text = subtext
		subtitle.TextColor3 = color.Dark(uipallet.Text, 0.48)
		subtitle.TextSize = 14
		subtitle.FontFace = uipallet.FontLight
		subtitle.Parent = object
		object.MouseEnter:Connect(function() object.BackgroundColor3 = color.Dark(uipallet.Main, 0.14) end)
		object.MouseLeave:Connect(function() object.BackgroundColor3 = color.Dark(uipallet.Main, 0.08) end)
		object.MouseButton1Click:Connect(callback)
	end

	function mainapi:RefreshConfigPanel()
		for _, object in cards:GetChildren() do
			if object:IsA('GuiObject') then object:Destroy() end
		end
		local order = 0
		card('Your Configs', 'Save and load local profiles with .config save/load', function() end, order)
		order += 1
		for _, profile in mainapi.Profiles do
			if profile.Name ~= 'default' then
				order += 1
				card(profile.Name, 'Click to load', function()
					mainapi:Load(true, profile.Name)
					feedback.Text = 'Loaded config: '..profile.Name
				end, order)
			end
		end
		local scriptpath = 'rise/scripts'
		if isfolder and not isfolder(scriptpath) and makefolder then
			makefolder(scriptpath)
		end
		local files = listfiles and isfolder and isfolder(scriptpath) and listfiles(scriptpath) or {}
		order += 1
		card('Scripts', #files..' local script(s) in rise/scripts', function() end, order)
		for _, path in files do
			if path:lower():sub(-4) == '.lua' then
				order += 1
				local name = path:match('[^/\\]+$') or path
				card(name, 'Click to load script', function()
					local success, result = pcall(function()
						return loadstring(readfile(path), name)(mainapi)
					end)
					feedback.Text = success and 'Loaded script: '..name or 'Script failed: '..tostring(result)
				end, order)
			end
		end
		cards.CanvasSize = UDim2.fromOffset(0, cardlayout.AbsoluteContentSize.Y + 10)
	end

	function mainapi:ExecuteCommand(input)
		local args = string.split(input:gsub('^%s+', ''):gsub('%s+$', ''), ' ')
		if args[1]:sub(1, 1) == '.' then args[1] = args[1]:sub(2) end
		local commandname = (args[1] or ''):lower()
		if commandname == 'config' or commandname == 'cfg' then
			local action = (args[2] or ''):lower()
			if action == 'save' and args[3] and args[3] ~= '' then
				mainapi:Save(args[3])
				feedback.Text = 'Saved config: '..args[3]
				if mainapi.RefreshCaS then mainapi:RefreshCaS() end
			elseif action == 'load' and args[3] and args[3] ~= '' then
				mainapi:Load(true, args[3])
				feedback.Text = 'Loaded config: '..args[3]
				if mainapi.RefreshCaS then mainapi:RefreshCaS() end
			elseif action == 'delete' and args[3] and args[3] ~= 'default' then
				for index, profile in mainapi.Profiles do
					if profile.Name == args[3] then table.remove(mainapi.Profiles, index) end
				end
				if delfile and isfile('rise/profiles/'..args[3]..mainapi.Place..'.txt') then delfile('rise/profiles/'..args[3]..mainapi.Place..'.txt') end
				if mainapi.Profile == args[3] then mainapi.Profile = 'default' end
				mainapi:Save()
				feedback.Text = 'Deleted config: '..args[3]
				if mainapi.RefreshCaS then mainapi:RefreshCaS() end
			elseif action == 'list' then
				local names = {}
				for _, profile in mainapi.Profiles do table.insert(names, profile.Name) end
				feedback.Text = #names > 0 and 'Configs: '..table.concat(names, ', ') or 'No configs found'
			else
				feedback.Text = 'Use .config save/load/delete/list <name>'
			end
		elseif commandname == 'script' or commandname == 'scripts' then
			local action = (args[2] or ''):lower()
			if action == 'list' then
				local names = {}
				local scriptfiles = listfiles and isfolder and isfolder('rise/scripts') and listfiles('rise/scripts') or {}
				for _, path in scriptfiles do table.insert(names, path:match('[^/\\]+$') or path) end
				feedback.Text = #names > 0 and 'Scripts: '..table.concat(names, ', ') or 'No scripts found'
			elseif action == 'load' and args[3] and listfiles and isfolder and isfolder('rise/scripts') then
				local wanted = args[3]:lower()
				local scriptfiles = listfiles('rise/scripts')
				for _, path in scriptfiles do
					if path:lower():match('[^/\\]+$') == wanted or path:lower():match('[^/\\]+$') == wanted..'.lua' then
						local success, result = pcall(function() return loadstring(readfile(path), path)(mainapi) end)
						feedback.Text = success and 'Loaded script: '..args[3] or 'Script failed: '..tostring(result)
						break
					end
				end
			else
				feedback.Text = 'Use .script list or .script load <name>'
			end
		elseif commandname == 'help' then
			feedback.Text = '.config save/load/delete/list <name> | .script list/load <name>'
		else
			feedback.Text = 'Unknown command. Try .help'
		end
		command.Text = ''
		mainapi:RefreshConfigPanel()
	end
	command.FocusLost:Connect(function(enter)
		if enter then mainapi:ExecuteCommand(command.Text) end
	end)
	mainapi:Clean(inputService.InputBegan:Connect(function(inputObj)
		if inputService:GetFocusedTextBox() or mainapi.Binding then return end
		if inputObj.KeyCode == Enum.KeyCode.Period then
			panel.Visible = true
			mainapi:RefreshConfigPanel()
			command:CaptureFocus()
		end
		if panel.Visible and inputObj.KeyCode == Enum.KeyCode.Escape then
			panel.Visible = false
			command:ReleaseFocus()
		end
	end))
	mainapi.ConfigPanel = panel
	mainapi.CommandInput = command
	mainapi:RefreshConfigPanel()
end
