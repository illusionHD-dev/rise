...
local function wipeFolder(path)
	...
	for _, file in listfiles(path) do
		...
		if isfile(file) and select(1, readfile(file):find('--This watermark is used to delete the file if its cached, remove it to make the file persist after rise updates.')) == 1 then
			delfile(file)
		end
	end
end
...
if not shared.RiseDeveloper then
	local _, subbed = pcall(function()
		return game:HttpGet('https://github.com/illusionHD-dev/rise')
	end)
	...
	if (isfile('rise/profiles/asset.txt') and readfile('rise/profiles/asset.txt') or '') ~= assetVer then
		wipeFolder('rise/assets')
	end

	writefile('rise/profiles/asset.txt', assetVer)
